...
${MODULE_ARMEL}:
	...
	mv -f ${GOPATH}/bin/linux_arm/go-google-gateway ${GOPATH}/go-google-gateway-0.0.1-SNAPSHOT.armel

${MODULE_ARMHF}:
	# module
	CC=arm-linux-gnueabihf-gcc GOOS=linux GOARCH=arm GOARM=7 CGO_ENABLED=1 CGO_CFLAGS="" go install -installsuffix armhf
	mv -f ${GOPATH}/bin/linux_arm/go-google-gateway ${GOPATH}/go-google-gateway-0.0.1-SNAPSHOT.armhf
